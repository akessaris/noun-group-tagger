...
#Run the following command: python ank352_HW5_train.py ../WSJ_CHUNK_FILES/WSJ_02-21.pos-chunk
...
trainFile = open("training.feature", "w+")
...
def makePairs(file):
    ...
    trainOutput = "" #Output to training file
    ...
    for line in file:
        ...
        if (line == "\n"):
            ...
            #Add blank lines to training output
            trainOutput += "\n"
...
#        nameArr = isName(prevFeatArr, currFeat, nextFeatArr)
#        prevFeatArr = nameArr[0]
#        currFeat = nameArr[1]
#        nextFeatArr = nameArr[2]
...
#        nextFeatArr = orgArray[2]
        ...
        trainOutput += currFeat["word"] + sep + "POS=" + currFeat["pos"] + sep + "cap=" + currFeat["cap"]
        
        #If not at beginning of sentence, add features of previous words
        for index, feat in enumerate(prevFeatArr):
            if (feat["word"] != ""):
                trainOutput += featToString("prev", str(index+1), feat)

        #If not at end of sentence, add features of next words
        for index, feat in enumerate(nextFeatArr):
            if (feat["word"] != ""):
                trainOutput += featToString("next", str(index+1), feat)

        #Add bio tag at end of line
        trainOutput += sep + currFeat["bio"]
    ...
    #Write feature value pairs (with bio tags) to test file
    trainFile.write(trainOutput)
...
def isName(prevFeatArr, currFeat, nextFeatArr):
    ...
    #Check next word after current
    if (currFeat["name"] == "true" and nextFeatArr[0]["cap"] == "true"):
